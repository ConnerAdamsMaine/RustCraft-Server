...
impl RegionPos {
    ...
    pub fn chunk_offset(&self, chunk_x: i32, chunk_z: i32) -> Option<usize> {
        ...
        if local_x >= 0 && local_x < REGION_SIZE && local_z >= 0 && local_z < REGION_SIZE {
            Some((local_z * REGION_SIZE + local_x) as usize)
        } else {
            None
        }
    }
...
impl Region {
    pub fn new(pos: RegionPos) -> Self {
        Self {
            pos,
            chunks: vec![None; (REGION_SIZE * REGION_SIZE) as usize],
            modified: false,
        }
    }
    ...
    pub fn serialize(&self) -> Vec<u8> {
        let serialized: Vec<SerializedChunk> = self
            .chunks_iter()
            .map(|chunk| SerializedChunk::from_chunk(chunk))
            .collect();
